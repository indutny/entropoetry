...
const MARKOV_STRING = require('./data/markov.json');
const MARKOV_TABLE = JSON.parse(MARKOV_STRING);
const ALL_WORDS = MARKOV_TABLE[''].map(row => row[0])
...
Poetry.prototype.stringify = function stringify(buf) {
  ...
  while (bitsLeft !== 0 || codes !== false) {
    ...
    this._context[0] = this._context[1];
    this._context[1] = word;
...
Poetry.prototype._huffman = function _huffman() {
  ...
  let table = markov[this._context.join(':')];
  if (!table) {
    this._debug('single word context', this._context);
    table = markov[this._context.slice(1).join(':')];
  }
...
Poetry.prototype._filter = function _filter(word) {
  ...
  const syllables = this._syllables + stress.length;
  if (syllables > this.metre.length)
    return false;

  const needRhyme = syllables === this.metre.length;
  ...
  const metreOffset = this._syllables;
